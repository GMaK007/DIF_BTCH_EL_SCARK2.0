...
class el_f2frt_mn_v2 {
  ...
  def defFields (rSS: SparkSession, rinfileNm: String): DataFrame = {
    ...
    val defVRowRDD = sc.parallelize(Seq(Row("NA", "", -1, rinfileNm.split("/").last, TODAY)))
...
object el_f2frt_mn_v2 {
  def main(args: Array[String]): Unit = {
    ...
    //delimit starts here
    val hdr = {
      if (appConf.getConfig(regn).getString("in.file.header") == "Y") {
        true
      } else {
        false
      }
    ...
    val infClmns = {
      if (!JSON) {
        ...
        spark.read.format("csv").option("delimiter", appConf.getConfig(regn).getString("in.file.delimiter")).option("header", hdr).load(infileNm).columns
      } else {
    ...
    val filedf = if (!JSON) {
      spark.read.format("csv").option("delimiter", appConf.getConfig(regn).getString("in.file.delimiter")).option("header", hdr).option("inferSchema","true").load(infileNm)
    } else {
...
//    for (i <- 0 until sClmnNms.length) { new_sClmnNms += (sClmnNms(i)+"_"+(i+1).toString)}
    ...
    if (TABLE) {
      ...
      val jdbc_url = s"${jdbcDBPlatform}${jdbcHostname}${jdbcPort}${jdbcDatabase}"
      ...
      try {
      ...
      catch
        {
          case e: IllegalArgumentException =>
            println(s"maybecolumn#mismatch ${e}")
            System.exit(1)
          //            val finaldf = findf.toDF() //.toDF(sqlTableClmns: _*)
